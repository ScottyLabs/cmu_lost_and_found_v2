import React from "react";
import { Icon, Label, Menu, Table } from "semantic-ui-react";

const TableExample = () => {
  return (
    <Table celled>
      <Table.Header>
        <Table.Row>
          <Table.HeaderCell>Date Found</Table.HeaderCell>
          <Table.HeaderCell>Time Found</Table.HeaderCell>
          <Table.HeaderCell>Object</Table.HeaderCell>
          <Table.HeaderCell>Where Found</Table.HeaderCell>
          <Table.HeaderCell>Description</Table.HeaderCell>
          <Table.HeaderCell>Category</Table.HeaderCell>
          <Table.HeaderCell>Where to Retrieve</Table.HeaderCell>
          <Table.HeaderCell>More Information</Table.HeaderCell>
        </Table.Row>
      </Table.Header>

      <Table.Body>
        <Table.Row>
          <Table.Cell>11/10/2019</Table.Cell>
          <Table.Cell>12:59 am</Table.Cell>
          <Table.Cell>Phone</Table.Cell>
          <Table.Cell>Tepper</Table.Cell>
          <Table.Cell>Small pink iPhone</Table.Cell>
          <Table.Cell>Phones</Table.Cell>
          <Table.Cell>Gates</Table.Cell>
          <Table.Cell>
          </Table.Cell>
        </Table.Row>
        <Table.Row>
          <Table.Cell>11/10/2019</Table.Cell>
          <Table.Cell>12:59 am</Table.Cell>
          <Table.Cell>Phone</Table.Cell>
          <Table.Cell>Tepper</Table.Cell>
          <Table.Cell>Small pink iPhone</Table.Cell>
          <Table.Cell>Phones</Table.Cell>
          <Table.Cell>Gates</Table.Cell>
          <Table.Cell>
          </Table.Cell>
        </Table.Row>
        <Table.Row>
          <Table.Cell>11/10/2019</Table.Cell>
          <Table.Cell>12:59 am</Table.Cell>
          <Table.Cell>Phone</Table.Cell>
          <Table.Cell>Tepper</Table.Cell>
          <Table.Cell>Small pink iPhone</Table.Cell>
          <Table.Cell>Phones</Table.Cell>
          <Table.Cell>Gates</Table.Cell>
          <Table.Cell>
          </Table.Cell>
        </Table.Row>
        <Table.Row>
          <Table.Cell>11/10/2019</Table.Cell>
          <Table.Cell>12:59 am</Table.Cell>
          <Table.Cell>Phone</Table.Cell>
          <Table.Cell>Tepper</Table.Cell>
          <Table.Cell>Small pink iPhone</Table.Cell>
          <Table.Cell>Phones</Table.Cell>
          <Table.Cell>Gates</Table.Cell>
          <Table.Cell>
          </Table.Cell>
        </Table.Row>
      </Table.Body>

      {/* <Table.Footer>
      <Table.Row>
        <Table.HeaderCell colSpan="3">
          <Menu floated="right" pagination>
            <Menu.Item as="a" icon>
              <Icon name="chevron left" />
            </Menu.Item>
            <Menu.Item as="a">1</Menu.Item>
            <Menu.Item as="a">2</Menu.Item>
            <Menu.Item as="a">3</Menu.Item>
            <Menu.Item as="a">4</Menu.Item>
            <Menu.Item as="a" icon>
              <Icon name="chevron right" />
            </Menu.Item>
          </Menu>
        </Table.HeaderCell>
      </Table.Row>
    </Table.Footer> */}
    </Table>
  );
}

export default TableExample;
